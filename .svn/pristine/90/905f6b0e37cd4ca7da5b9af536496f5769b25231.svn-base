...
class ParentModel extends AppModel {
    ...
    /*增加家长接送一览照片*/
    function getPickUpPicList($sh,$all = false){
        $schoolId = $_SESSION[SESSION_USER]["school_id"];
        $this->setSearch($sh);

        $sql = "
            SELECT tb_away.img AS path,tb_away.sub_img AS subPath,tb_away.created AS createTime,tb_student.name FROM tb_student_away_record AS tb_away
            LEFT JOIN tb_student ON tb_away.student_id = tb_student.id
            WHERE tb_student.school_id = '{$schoolId}' AND tb_student.deleted = 0";

        $this->where($sql, 'tb_away.created', 'date_time', '>=', 'AND');
        $this->where($sql, 'tb_away.created', 'date_time1', '<', 'AND');
        $this->where($sql, 'tb_student.name', 'name', '=','AND');
        // $this->order($sql, 'order.default');


        if($all){
            $this->order($sql, 'order.takeAway');
            $rs = $this->getAll($sql);
        }else{
            $this->order($sql, 'order.takeAway');
            $this->paging('paging.imgPaging');
