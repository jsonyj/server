...
abstract class ApnsPHP_Abstract
{
	 ...
	 */
	public function __construct($nEnvironment, $sProviderCertificateFile,$passphrase = '')
	{
	...
	protected function _connect()
	{
		...
		if ($this->_passphrase) {
			$option['ssl']['passphrase'] = $this->_passphrase;
		}
		$streamContext = stream_context_create($option);
	...
	protected function _log($sMessage)
	{
		if (!isset($this->_logger)) {
			$this->_logger = new ApnsPHP_Log_MyLog();
		}
